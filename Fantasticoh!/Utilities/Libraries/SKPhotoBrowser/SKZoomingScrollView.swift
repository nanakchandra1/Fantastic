    ...
    open func displayImage(complete flag: Bool) {
        ...
        if let url = URL(string: photo.photoURL){
            ...
            if let image = photo.photoURL {
                // image
             //   photoImageView.image = #imageLiteral(resourceName: "placeholder_media")
                indicatorView.startAnimating()
               // photoImageView.sd_setImage(with: URL(string: image), placeholderImage: #imageLiteral(resourceName: "placeholder_media")) //image = image
//                photoImageView.sd_setImage(with: URL(string: image), completed: { (imagePost, error, state, link) in
//                    
//                    self.indicatorView.stopAnimating()
//                })
                
//                photoImageView.sd_setImage(with: URL(string: image), placeholderImage: CONTAINERPLACEHOLDER, options: .continueInBackground, completed: { (img, error, tempCatch, url) in
//                     self.indicatorView.stopAnimating()
//                })
                
                photoImageView.sd_setImage(with: URL(string: image), placeholderImage: transparentPLACEHOLDER, options: .highPriority, completed: { (imagePost, error, state, link) in
                    if imagePost != nil {
                        self.photoImageView.contentMode = .scaleAspectFit
                      self.photo.checkCache()
                    }
                    self.indicatorView.stopAnimating()
                })
                ...
                photoImageViewFrame.size = photoImageView.image?.size ?? (transparentPLACEHOLDER?.size)!
            ...
            if let image = photo.underlyingImage {
                ...
                var photoImageViewFrame = CGRect.zero
                photoImageViewFrame.origin = CGPoint.zero
                photoImageViewFrame.size = photoImageView.image!.size
